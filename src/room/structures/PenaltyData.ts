...
export type PenaltyName = keyof PENALTY_TYPES;

export interface PENALTY_TYPES {
  snapOutOfHashes: void;
  snapOutOfBounds: void;
  offsidesOffense: {
    player: PlayerObject;
  };
  offsidesDefense: {
  ...
  kickOffDrag: void;
  kickOffDragSafety: void;
  kickOffOutOfBounds: void;
  kickOffOutOfBoundsSafety: void;
  kickOffOffsides: {
    player: PlayerObject;
  };
...
export default class PenaltyData<T extends PenaltyName> {
  private _getPenalty(penaltyName: PenaltyName, penaltyData: PENALTY_TYPES[T]) {
    const { playerName = "", time = 0 } = penaltyData as unknown as {
      playerName?: string;
      time?: number;
    };
  ...
  getData(
    penaltyName: keyof PENALTY_TYPES,
    ...
    const {
      ...
      addDown,
      hasOwnHandler = false,
    } = this._getPenalty(penaltyName, penaltyData);
